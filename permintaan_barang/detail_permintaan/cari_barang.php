...
<form action="cari_barang.php" enctype="multipart/form-data" name="barang" method="post" >
  <table border="0" class="table table-striped">
    <tr>
	  ...
	  <input type="hidden" name="no_permintaan_barang_f" value="<?php echo $no_permintaan_barang; ?>"
	  </td>
...
<?php	
	...
	if (isset($_POST['kirim']))
	{
		...
		$no_permintaan_barang	= isset($_POST['no_permintaan_barang_f']) ? $_POST['no_permintaan_barang_f'] : '';
		...
		foreach($masukdetailpermintaan->Que($que) as $value)
			...
			echo '
				  <tr>
					...
					<td> '.$value["satuan"].' </td>
					<td align="center"><a href="detail_permintaan.php?no_permintaan_barang='.$no_permintaan_barang.'&id_brg='.$value["id_brg"].'&nama_barang='.$value["nama_barang"].' "><input type="button" class="form-control" value="Pilih"></a></td>
				  </tr>
				  ';
			}		
		}
		...
		elseif($kategori == "nol")
		{
		$que = "select * from barang, jenis_barang where barang.id_jenis_barang=jenis_barang.id_jenis order by jenis_barang.jenis_brg asc";
		foreach($masukdetailpermintaan->Que($que) as $value)
			{ 
			echo '
				  <tr>
